...
## 📤 Push & Deploy (Git + Docker + Compose)
This project includes quick instructions for pushing code to git, publishing Docker images to a registry, and deploying locally with Docker Compose. All example commands below are written for PowerShell on Windows — adjust for your shell as needed.
...
```
2. Push to your remote repository (replace branch and remote as needed):
```powershell
git push origin main
```

### B) Docker Hub (or another registry) — build, tag, push
Prerequisites: docker login (to Docker Hub or your registry)
```powershell
# Build the order-service image locally (from repo root)
docker build -t YOUR_DOCKERHUB_USERNAME/order-service:latest ./order-service

# Optional: add a version tag
docker tag YOUR_DOCKERHUB_USERNAME/order-service:latest YOUR_DOCKERHUB_USERNAME/order-service:v1.0.0

# Push to Docker Hub (or your registry)
docker push YOUR_DOCKERHUB_USERNAME/order-service:latest
# docker push YOUR_DOCKERHUB_USERNAME/order-service:v1.0.0
```
Notes:
- Replace `YOUR_DOCKERHUB_USERNAME` with your Docker Hub username (or a registry hostname for other registries).
- For GitHub Container Registry (ghcr.io) or other registries, use the appropriate image name and login flow.

### C) Docker Compose — build & deploy locally
From the project root (where `docker-compose.yml` lives):
```powershell
# Build and start all services in detached mode
docker-compose up --build -d

# Start/replace only the order-service after code changes
docker-compose up --build -d order-service

# View running containers
docker ps

# Follow logs for order-service
docker-compose logs -f order-service

# Stop and remove the compose stack
docker-compose down

# To also remove data volumes (WARNING: deletes DB data)
docker-compose down --volumes
```

### D) Important notes for this repo
- `docker-compose.yml` maps host ports (e.g., Postgres 5433 -> container 5432, order-service 8080 -> 8080). When services run inside Docker, they must connect to other containers using the compose service names (e.g., `postgres:5432`, `kafka:29092`).
- The project contains a Docker-specific Spring profile file: `order-service/src/main/resources/application-docker.yaml`. Compose sets `SPRING_PROFILES_ACTIVE=docker`, so the service will pick up the proper container hostnames when running under Docker.
- Postman collection: `OrderService.postman_collection.json` is in the repo root — import this into Postman and set the `baseUrl` variable to `http://localhost:8080`.
